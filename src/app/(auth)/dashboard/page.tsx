...
export default function DashboardPage() {
  ...
  async function loadData() {
    ...
    const [pedidos, orcamentos, appts, clientes, docs] = await Promise.all([
    ...
    // Calculate faturamento and deadlines
    const { data: allPedidos } = await supabase.from('document').select('doc_data, status, date').eq('type', 'pedido')
    let fat = 0
    let venc = 0
    let hoje = 0
    if (allPedidos) {
      for (const p of allPedidos) {
        const val = parseFloat((p.doc_data?.productValue || '').replace(/[^\d,]/g, '').replace(',', '.')) || 0
        if (['pago', 'instalado'].includes(p.status)) fat += val
        // Check deadline
        ...
        if (match && date) {
          ...
          if (parts.length === 3) {
            ...
            if (diffDays < 0 && !['pago', 'instalado', 'cancelado'].includes(p.status)) venc++
            else if (diffDays === 0 && !['pago', 'instalado', 'cancelado'].includes(p.status)) hoje++
          }
        }
      }
    }
